...
import com.hmdp.config.KafkaTopicConfig;
...
//        return Result.ok(orderId);
        // 2. 脱离请求线程，发消息给 Kafka
        // 用 voucherId 作为 key：保证同一张券的所有订单进入同一分区，方便分区内顺序消费
        ...
        try {
            kafkaTemplate.send(KafkaTopicConfig.SECKILL_ORDER_TOPIC, voucherId.toString(), jsonStr);
        } catch (Exception e) {
